...
`1.5.3`_
--------
- Made constructing a :class:`~.Component` by name/identifier produce the same fully-initialized component as :meth:`~.Component.from_chemical`, including for phase-overridden species. Previously ``Component(ID)`` / ``Component(ID, search_ID=...)`` built the underlying chemical natively and then locked the phase *after* its energies were initialized at the chemical's natural phase, so a component forced to a non-natural phase kept an enthalpy model referenced to the wrong phase. The symptoms were a wrong (often non-zero) reference enthalpy or, for a species whose natural phase is liquid forced to solid (e.g. ``K+``), a ``TypeError`` (``float - NoneType``) crash during stream mixing in ``simulate()``. The constructor now passes ``phase`` into the native build so Thermosteam performs its canonical lock-then-init order (matching ``from_chemical``); phase-overridden components are now simulate-safe and report ``H = 0`` at the reference temperature. ``search_ID`` and ``chemical`` were given typed roles: ``search_ID`` takes a name string to look up (an explicit miss raises ``LookupError``; a bare ``ID`` miss still falls back to a blank component to fill in manually), and ``chemical`` takes a pre-built ``thermosteam.Chemical`` object (a name string is accepted as an alias for ``search_ID``); passing a ``Chemical`` to ``search_ID`` raises ``TypeError`` and giving both a ``Chemical`` object and a ``search_ID`` raises ``ValueError``. ``Component.from_chemical`` is retained as a thin backward-compatible wrapper. One behavior change: ``from_chemical(chemical='<name>', formula=<override>)`` now reports the molecular weight of the override ``formula`` (consistent with ``Component(ID, search_ID=..., formula=...)``) instead of the database chemical's MW; the ``Struvite`` docstring example was updated accordingly. Added ``tests/test_component.py`` coverage for the kwarg contract, the phase-override fix, the blank-component fallback, the formula override, and a drift-guard test that pins the hand-rolled object copy against Thermosteam's ``Chemical.copy``.

- Added a ``formula_override`` argument to :class:`~.Component` (and :meth:`~.Component.from_chemical`), default ``False``, that gates overriding a database/source chemical's formula. When a component is built from a resolved chemical (via ``search_ID`` or ``chemical``) and a ``formula`` with a *different atomic composition* is given, construction now raises ``ValueError`` unless ``formula_override=True`` is passed; a different spelling of the same atoms (e.g. ``CH3CH2CH3`` for ``C3H8``) is not treated as an override and needs no flag, and a blank custom component (no source chemical) is never gated. This catches accidental formula/lookup mismatches while keeping the legitimate "borrow a chemical's thermodynamic models but substitute a corrected formula/MW" pattern available explicitly. The formula re-initialization (recomputing ``chem_MW`` and the element-content fractions from the new formula) was unified into a single ``Component._apply_formula`` helper used by both construction paths. In EXPOsan, the components that legitimately override a looked-up chemical's formula now pass ``formula_override=True``: ``Struvite`` (bwaise, htl), ``FilterBag`` (biogenic_refinery), and ``Zeolite`` (reclaimer).

- Added runnable docstring examples to the :class:`~.unit_operations.Excretion`, :class:`~.unit_operations.PitLatrine`, and :class:`~.unit_operations.Trucking` sanitation units (previously their ``Examples`` sections only linked to EXPOsan), and a supporting :func:`qsdsan.utils.create_example_sanitation_components` helper that builds the excreta/nutrient component set those units need (it is not part of ``Components.load_default``). Also fixed the :class:`~.unit_operations.Incinerator` docstring example, whose ``Component.from_chemical('S_CO2', search_ID='CO2', ...)`` call passed a ``search_ID`` keyword that ``from_chemical`` does not accept (it was silently swallowed by ``**data``, so the chemical defaulted to the ID ``'S_CO2'`` and the lookup raised ``LookupError``); it now uses the supported ``chemical='CO2'`` form. These examples run under ``pytest --doctest-modules`` (already exercised in CI via the ``pytest.ini`` ``--doctest-modules`` default).

- Extended the docstring-example coverage to more built-in unit operations. Added runnable ``Examples`` to the sanitation/static units :class:`~.unit_operations.MURT`, :class:`~.unit_operations.UDDT`, :class:`~.unit_operations.CropApplication`, :class:`~.unit_operations.Lagoon`, :class:`~.unit_operations.SepticTank`, :class:`~.unit_operations.DryingBed`, :class:`~.unit_operations.LiquidTreatmentBed`, :class:`~.unit_operations.Screening`, :class:`~.unit_operations.Sedimentation`, and :class:`~.unit_operations.SludgePasteurization`, and to the wastewater-treatment units :class:`~.unit_operations.ActivatedSludgeProcess`, :class:`~.unit_operations.BiogasCombustion`, :class:`~.unit_operations.CombinedHeatPower`, :class:`~.unit_operations.AnaerobicBaffledReactor`, :class:`~.unit_operations.AnaerobicDigestion`, :class:`~.unit_operations.SludgeThickening`, :class:`~.unit_operations.BeltThickener`, and :class:`~.unit_operations.SludgeCentrifuge`. Added :func:`qsdsan.utils.create_example_wwt_components` (a lumped COD-based component set with the ``active_biomass``/``inert_biomass``/``substrates`` groups those units expect) and extended :func:`qsdsan.utils.create_example_sanitation_components` with ``Struvite``, ``HAP``, ``MagnesiumHydroxide``, and ``LPG`` so the struvite-recovery and pasteurization examples run. The BioSTEAM-wrapper units in ``qsdsan.unit_operations.bst`` now all link to the upstream BioSTEAM API documentation from their ``See Also`` sections (the :class:`~.unit_operations.Tank` family gained the links the others already had). All examples run under ``pytest --doctest-modules``.
