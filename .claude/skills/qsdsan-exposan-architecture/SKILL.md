...
Built-in units live in `qsdsan.unit_operations`, split into three sub-namespaces. `bst` answers a different question than `static`/`dynamic` do, so decide it first and separately — it's not a competing rule that "overrides" a capability test, it's a different axis:

1. `bst`: the class is a thin wrapper whose entire purpose is making a real `biosteam.units` class simulate-able inside QSDsan (`Mixer`, `Splitter`, `Tank`, `Pump`, etc. — same core BioSTEAM simulate/design/cost logic, QSDsan only adds stream-type compatibility and, where present, DAE interoperability plumbing so the wrapper can sit inside a dynamic flowsheet next to real dynamic units). `Mixer`/`Splitter` implement `_compile_AE` for exactly that plumbing reason, not because they have dynamic behavior of their own — so their presence in `bst` isn't a capability question at all. A class that merely *subclasses* one of these wrappers to reuse its mechanics for a genuinely new, QSDsan-native unit (e.g. `ESAPRecovery(Splitter)`) is not "a thin BioSTEAM wrapper" — it's a QSDsan unit, so classify it by rule 2/3 below.
2. `dynamic`: (everything not caught by rule 1) the class **can run in dynamic mode without erroring** — i.e. `isdynamic=True` actually works, whether that capability is implemented directly or inherited: `_init_state` plus `_compile_ODE` or `_compile_AE`, `_update_state`/`_update_dstate` are all present and functional. This is a pure runtime-capability test. None of the following disqualify a class from `dynamic`: `isdynamic` defaulting to `False`, the class also having legitimate steady-state design/costing, or the docstring never showing a dynamic example. If it would run without error under `isdynamic=True`, it belongs in `dynamic` — full stop.
3. `static`: everything else — units that would error if you tried to run them dynamically (no working dynamic-state contract).

A file containing a mix of dynamic-capable and static-only classes must be split by class, not left merged (precedent: `dynamic/_anaerobic_reactor.py` holds `AnaerobicCSTR`, split out from `static/_anaerobic_reactor.py`'s `AnaerobicBaffledReactor`/`AnaerobicDigestion`/`SludgeDigester`). Several files were renamed into `static/` wholesale during the original refactor without this split and are misclassified as a result (fixed 2026-07-07: `_clarifier.py`, `_membrane_gas_extraction.py`, `_metal_dosage.py`, `_sludge_treatment.py` moved to `dynamic/` in full; `_electrochemical_cell.py` and `_excretion.py` split by class). Don't treat current file location as ground truth going forward either — check each class against the capability test above, not where it happens to sit.
