...
from .. import Stream, SanUnit, process_models as pc
...
class mADM1toASM2d(mADMjunction):
    ...
    downstream : stream or str
        Effluent stream with ASM components.
    adm1_model : obj
        The anaerobic digestion process model (:class:`qsdsan.process_models.ADM1_p_extension`).
    rtol : float
        Relative tolerance for COD and TKN balance.
    atol : float
        Absolute tolerance for COD and TKN balance.
...
class mASM2dtoADM1p(A1junction):
    ...
    downstream : stream or str
        Effluent stream with ADM components.
    adm1_model : :class:`qsdsan.process_models.ADM1_p_extension`
    ...
    def _compile_reactions(self):
        ...
        asm = self.asm2d_model
        adm = self.adm1_model
        p1a_stoichio = np.array(asm.stoichiometry.loc['hetero_growth_S_A'], copy=True)
        p1a_stoichio /= abs(p1a_stoichio[S_O2_idx])
        p1f_stoichio = np.array(asm.stoichiometry.loc['hetero_growth_S_F'], copy=True)
        p1f_stoichio /= abs(p1f_stoichio[S_O2_idx])
        p2a_stoichio = np.array(asm.stoichiometry.loc['denitri_S_A'], copy=True)
        p2a_stoichio /= abs(p2a_stoichio[S_NO3_idx])
        p2f_stoichio = np.array(asm.stoichiometry.loc['denitri_S_F'], copy=True)
