...
class AnaerobicCSTR(CSTR):
    ...
    .. [3] Metcalf & Eddy Inc., Tchobanoglous, G., Burton, F. L., Tsuchihashi, R., 
        & Stensel, H. D. (2013). Wastewater engineering: Treatment and 
        resource recovery (5th ed.). McGraw-Hill Professional.
    .. [4] U.S. Environmental Protection Agency. (2006). Biosolids Technology Fact 
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None,
                 ...
                 bCOD=None, height=7.5, solids_conversion_efficiency=0.7, 
                 sludge_solids=0.05, organic_loading=2.57,
        ...
        self.T = T
        # self._S_gas = None
        self.design_flowrate = design_influent_flowrate
        ...
        self.height = height
        self.solids_conversion_efficiency = solids_conversion_efficiency
        self.sludge_solids = sludge_solids
        self.organic_loading = organic_loading
    ...
    def _design(self):
        ...
        cmps = self.components
        Q, sludge_mass, bCOD = self.design_flowrate, self.sludge_mass, self.bCOD
        if Q is None: Q = inf.F_vol * 24 # m3/hr to m3/day
        if sludge_mass is None: sludge_mass = inf.F_mass * 24 # kg/hr to kg/day
        if bCOD is None: 
            bCOD = 0
            for cmp in cmps:
                if cmp.degradability == 'Readily' or cmp.degradability == 'Biological':
                    if inf.imass[cmp.ID] != 0:
                        bCOD += inf.imass[cmp.ID] * 24 / Q  # kg/hr to g/L 

        sludge_volume = sludge_mass/(1.02 * 1000 * self.sludge_solids)
        ...
        V_dig_sludge = max(sludge_volume * self.SRT, Q * bCOD/self.organic_loading)
        P_solids = Y * Q * (bCOD - bCOD_eff)/(1 + (b * self.SRT))
        V_CH4 = 0.4 * ((Q * (bCOD - bCOD_eff)) - (1.42 * P_solids))
        V_gas = V_CH4/0.65
        self.V = V_dig_sludge + V_gas

        D['Volumetric flow rate'] = Q # in m3/day
        D['Sludge retention time'] = self.SRT # in days
        D['Digester volume'] = self.V # in m3
        D['Digester height'] = self.height # in m
