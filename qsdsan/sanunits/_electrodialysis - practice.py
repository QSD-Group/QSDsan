...
#%%
    def _update_dstate(self):
        ...
        if eff_dc.dstate is None:
            eff_dc.dstate = np.zeros(n)
        if eff_ac.dstate is None:
            eff_ac.dstate = np.zeros(n)

        # self._state == [Sac_inf_dc, Set_inf_dc,....., Q_inf_dc,Sac_inf_ac, Set_inf_ac,....., Q_inf_ac]
        eff_dc.dstate[:] = self._dstate[:14]
        eff_ac.dstate[:] = self._dstate[14:]
        
        Q_dc = self._dstate[n_compounds_dc]
        Q_ac = self._dstate[-1]
        ...
        # mg COD/L below
        #!!! Adding below myself
        eff_dc.dstate[indices] = eff_dc.dstate[indices] - (self.J_T * self.A_m / Q_dc * 3600/1000) * mol2mass[indices] * 1000
        eff_ac.dstate[indices] = (self.J_T * self.A_m / Q_ac * 3600/1000 * (1 - np.exp(-Q_ac * self.t / self.V / 3600))) * mol2mass[indices] * 1000
...
#%%
    ...
    def _compile_AE(self):
        ...
        def yt(t, y_ins, dy_ins):
            ...
            _dstate[:n_cols] = dy_ins[0]
            _dstate[n_cols:] = dy_ins[1]
            _update_state()
            _update_dstate()
