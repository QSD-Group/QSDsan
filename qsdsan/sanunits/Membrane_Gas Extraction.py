...
class GasExtractionMembrane(SanUnit):
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None, isdynamic=False, 
                  init_with='WasteStream', F_BM_default=None,   FiberID=0.05, FiberOD=0.10, 
                  MemThick=0.02, NumTubes=5, ShellDia=0.5, SurfArea=400,   
                  GasID = ['H2', 'CO2', 'CH4', 'O2', 'N2', 'H2O'], PVac = 97.325, segs = 500, VolBatchTank=1000,
                  GasPerm = {}, HenryPreFac = {}, HenrySlope = {}, WilkeChang = {}):
        ...
        self.MemThick = MemThick      # Membrane Thickness [m]
        ...
        self.Volume = VolBatchTank    # Volume of the bioreactor
    ...
    # Calculate the effective length of the membrane by taking the ratio of the declared surface area, and dividing it by the area per length of the tube. 
    @property
    def Length(self):
    ...
    def Diff(self):
        ...
        PermC = 3.35*(10**(-16))    # Conversion b/w Barrer and SI Units
    ...
    def _compile_ODE(self):
        ...
        ShellAc = self.ShellAc      # Shell Cross-Sectional Area [m^2]
...
# =============================================================================
        ...
        #Cin[i] = GasVec[i].Cin()
        Cins = self.ins[0].conc/cmps.MW*cmps.i_mass # inlet molar concentration [mol/m^3]
        Cin = Cins[self.idx] # self.idx ensures that it is only for gases 
        MMs = cmps.MW
        MM = MMs[self.idx] # self.idx ensures that it is only for gases 
        ...
        #print(KLiq)
        #print(KTot)
        # Synthesize the output vector of concentration differences
        
        # Initialize
        C = self._state
        dC = self._dstate
        #dC = np.zeros(Segs*numGas*2)
        ...
        for j in range(0, numGas):
            ...
            if j == self.h2o_j:
                ...
                dC[j] = (u/dx)*(C[Segs*numVec+j] - C[j]) - (KTot[j]/(D/4))*(C[j] - (C[j+numGas]/sumCp_init)*P/H[j])
        ...
        # Calculate changes in tank concentration based the values of SS
        for j in range(0, numGas):
            if SS == 0:
                dC[Segs*numVec+j] = ((Q/60)/V)*(C[numVec*(Segs-1)+j] - C[j])
            if SS == 1:
                dC[Segs*numVec+j] = 0
