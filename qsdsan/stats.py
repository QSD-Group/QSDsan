...
def plot_morris_results(morris_dct, metric,
    ...
    or bar plots.
    In scatter plots, the x values are $mu^*$ and the y values are $\sigma$.
    In bar plots, bar length indicate the $mu^*$ values with error bars
    ...
    kind : str
        Either "scatter" ($mu^*$ vs. $\sigma$) or "bar" ($mu^*$ with confidence interval) plot.
    x_axis : str
