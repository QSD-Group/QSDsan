...
__all__ = ('create_pm2_asm2d_cmps', 'PM2ASM2d')

_path = ospath.join(data_path, 'process_data/_pm2_asm2d.tsv')
...
def create_pm2_asm2d_cmps(set_thermo=True):
    ...
    cmps_pm2_asm2d = Components([X_CHL, X_ALG, X_CH, X_LI, S_CO2, S_A, S_F, 
                           S_O2, S_NH, S_NO, S_P, X_N_ALG, X_P_ALG, 
                           S_N2, S_ALK, S_I, X_I, X_S, X_H, X_AUT, cmps.H2O])
    
    cmps_pm2_asm2d.default_compile()                   

    if set_thermo: settings.set_thermo(cmps_pm2_asm2d)
    return cmps_pm2_asm2d

# create_pm2_asm2d_cmps()                
...
def rhos_pm2_asm2d(state_arr, params):
...
class PM2ASM2d(CompiledProcesses):
    ...
    def __new__(cls, components=None,
        ...
        self.set_rate_function(rhos_pm2_asm2d)
