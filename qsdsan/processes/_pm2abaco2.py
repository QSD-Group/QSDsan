...
def create_pm2abaco2_cmps(set_thermo=True):
    ...
    '''added from abaco2'''
    # S_BSMO (g COD/m^3) - copied from ASM2d S_F
    S_BSMO = cmps.S_F.copy('S_BSMO')
    S_BSMO.description = ('Concentration of biodegradable soluble organic matter')

    # X_NIT (g COD/m^3) - copied from ASM2d X_AUT
    X_NIT = cmps.X_AOO.copy('X_NIT')
    X_NIT.description = ('Concentration of nitrifying bacteria biomass')
    
    # X_HET (g COD/m^3) - copied from ASM2d X_H
...
def rhos_pm2abaco2(state_arr, params):
    ...
    c_arr = state_arr[:17]
    temp = state_arr[18]
    light = state_arr[19]     # imported from input file assumed

    # Q = state_arr[17]                             # Flow rate
    # t = state_arr[18]                             # time

    X_CHL, X_ALG, X_PG, X_TAG, S_CO2, S_A, S_G, S_O2, S_NH, S_NO, S_P, X_N_ALG, X_P_ALG, S_BSMO, X_NIT, X_HET, H2O = c_arr
