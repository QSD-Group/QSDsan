...
def test_component():
    ...
    assert isclose(cmps3.S_NO2.i_COD, - molecular_weight({'O':6})/molecular_weight({'N':4}), rel_tol=1e-3)
    assert isclose(cmps3.S_NO3.i_COD, - molecular_weight({'O':10})/molecular_weight({'N':4}), rel_tol=1e-3)
    set_thermo(cmps3)
