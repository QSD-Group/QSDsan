...
def test_sanunit():
    ...
    M1 = qs.unit_operations.Mixer('M1', ins=(ws1, ws2, ''), outs='mixture',
                           init_with='WasteStream')
    M1.show()
    assert type(M1.ins[0]).__name__ == 'WasteStream'

    S1 = qs.unit_operations.Splitter('S1', ins=M1-0, outs=('', ''), split=0.2)
    ins3 = qs.WasteStream(S_CH3OH=7, H2O=1000, units='kg/hr')
    P1 = qs.unit_operations.Pump('P1', ins=ins3)
    M2 = qs.unit_operations.MixTank('M2', ins=(S1-0, P1-0), tau=2)
    ...
    ss2 = ss1.copy()
    M3 = qs.unit_operations.MixTank('M3', ins=ss1, init_with='WasteStream')
    M3.F_BM['Tank'] = 1
    M3.simulate()
    M3.show()
    ...
    M4 = qs.unit_operations.MixTank('M4', ins=ss2, init_with='Stream')
