...
def test_sanunit():
    ...
    from sanitation import Components, WasteStream, Mixer, Splitter, Pump, MixTank
    ...
    M1 = Mixer('M1', ins=(ws1, ws2, ''), outs='mixture')
    assert type(M1.ins[0]).__name__ == 'WasteStream'
    
    S1 = Splitter('S1', ins=M1-0, outs=('', ''), split=0.2)
    ins3 = WasteStream(SCH3OH=7, H2O=1000, units='kg/hr')
    P1 = Pump('P1', ins=ins3)    
    M2 = MixTank('M2', ins=(S1-0, P1-0), tau=2)
    M2-0-2-M1
